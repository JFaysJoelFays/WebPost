...
namespace WebPost.Models
{
    public class CommentService
    {
        ...
        {
            var filter = Builders<Post>.Filter.Eq("_id", postId);
            var filterComment = Builders<Comment>.Filter.Eq("_id", commentId);
            var update = Builders<Post>.Update.Pull("Comments", filterComment).Inc("TotalComments", -1);
        ...
        public IList<Comment> GetComments(ObjectId postId, int skip, int limit, int totalComments)
        {
            ...
            var filter = Builders<Post>.Filter.Eq("_id", postId);
            var projection = Builders<Post>.Projection.Exclude("Date").Exclude("Title").Exclude("Url").Exclude("Summary")
                    .Exclude("Details").Exclude("Author").Exclude("TotalComments").Slice("Comments", -skip, limit);
